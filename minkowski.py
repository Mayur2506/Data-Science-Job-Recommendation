...
from sklearn.feature_extraction.text import CountVectorizer
from sklearn.metrics.pairwise import pairwise_distances
def minkowski(user_experience,user_skills):
    ...
    # Calculate the Euclidean distance between the user's skills and the job skills
    euclidean_dist = pairwise_distances(skills_dense, user_skills_dense, metric='minkowski', p=2)

    # Add the similarity score to the DataFrame
    filtered_jobs_df = filtered_jobs_df.assign(similarity=(1 / (1 + euclidean_dist)).flatten())
