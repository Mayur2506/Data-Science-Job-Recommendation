...
def hamming(user_experience,user_skills):
    ...
    # Calculate the Hamming distance between the user's skills and the job skills
    hamming_dist = pairwise_distances(skills_dense, user_skills_dense, metric='hamming')

    # Add the similarity score to the DataFrame
    filtered_jobs_df = filtered_jobs_df.assign(similarity=1 - hamming_dist.flatten())
